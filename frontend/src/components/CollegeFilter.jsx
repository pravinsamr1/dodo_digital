...
const CollegeFilter = ({ onFilterChange }) => {
  ...
  const handleApply = () => {
    if (onFilterChange) {
      onFilterChange({ program, stream: selectedStream, type: collegeType, location, distance, fees });
    }
  ...
  const clearFilters = () => {
    ...
    setFees(150000);
    if (onFilterChange) {
      onFilterChange({ program: 'UG - PG', stream: 'Engineering', type: 'Private', location: userLocation, distance: 15, fees: 150000 });
    }
  };
