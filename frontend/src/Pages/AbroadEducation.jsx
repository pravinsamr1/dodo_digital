...
import AbroadEducationFilter from '../components/AbroadEducationFilter';
import { abroadEducation } from '../data/abroadEducation';
...
const AbroadEducation = () => {
  ...
  const [filters, setFilters] = useState({ type: 'All', destination: 'All' });
  ...
  const filteredItems = abroadEducation.filter(item => {
    if (filters.type !== 'All' && item.type !== filters.type) return false;
    if (filters.destination !== 'All' && !item.destinations.includes(filters.destination)) return false;
    return true;
  });
  ...
  return (
    <>
      <PageSEO title="Abroad Education" description="Find the best institutes to study and work abroad." />
      <div className="min-h-screen bg-slate-50 pb-10">
        <div
          className="relative mb-10 h-34 overflow-hidden bg-cover bg-center"
          style={{
            backgroundImage: "url('https://images.unsplash.com/photo-1521587760476-6c12a4b040da?auto=format&fit=crop&q=80&w=1600')",
          }}
        >
          <div className="absolute inset-0 bg-slate-950/70" />
          <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center">
            <h1 className="text-3xl font-[500] text-white md:text-5xl">Abroad Education</h1>
          </div>
        </div>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="flex flex-col gap-6 lg:flex-row">
            <div className="lg:w-72 lg:shrink-0">
              <AbroadEducationFilter onFilterChange={handleFilterChange} />
            </div>
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {item.features.map(feature => (
                             <span key={feature} className="bg-slate-100 text-slate-600 text-[10px] px-2 py-1 rounded-full">{feature}</span>
                          ))}
                        </div>
